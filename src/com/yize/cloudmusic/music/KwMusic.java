...
public class KwMusic implements MusicHelper {
    ...
    private String getDownloadLinkById(String songid,String quality){
        try {
            String link="http://nmobi.kuwo.cn/mobi.s?f=kuwo&q=";
            String bitRate="2000";
            String format="flac";
            if(quality.length()==0){
                quality="SQ";
            }
            if(quality.equals("LQ")){
                bitRate="128";
                format="mp3";
            }else if(quality.equals("PQ")){
                bitRate="192";
                ...
                bitRate="320";
                format="mp3";
            }else if(quality.equals("SQ")){
                bitRate="2000";
                format="flac";
            }
            String encParams = base64_encrypt("&type=convert_url2&br="+bitRate+"&format="+format+"&sig=0&rid="+songid+"&network=wifi");
            String response=downloadWebSite(link+encParams);
            String downloadLink=response.substring(response.indexOf("url=")+4,response.lastIndexOf("sig="));
            return downloadLink;
        } catch (Exception e) {
            //e.printStackTrace();
            return null;
